...
from urx_local import urrtmon
from urx_local import ursecmon
...
class URRobot(object):
    ...
    def _format_tpose(self, tpose):
        rv = tpose.rv.get_array()
        tpose = [tpose.p[0], tpose.p[1], tpose.p[2], rv[0], rv[1], rv[2]]
        return tpose

    def _format_move(self, command, tpose, acc, vel, radius=0, prefix=""):
        tpose = self._format_tpose(tpose)
