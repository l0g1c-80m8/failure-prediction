...
def activate(gripper,gripper_connected):
    ...
    if gripper_connected:
        ...
        time.sleep(0.01)
        data = binascii.hexlify(gripper.readline()) # read entire line to clear all data
        print(data)
...
def main():
    # Specify comm port:
    comm_default = '/dev/serial/by-id/usb-ROBOTIQ_INC._I_O_Coupling_333535383038510400460032-if00'
    if len(sys.argv)>1:
    ...
    # gripper.write(bytearray.fromhex("090603E801000962"))
    # data = binascii.hexlify(gripper.readline())
    # print(data)
