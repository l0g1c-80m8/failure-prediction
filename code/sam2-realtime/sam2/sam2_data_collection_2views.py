    ...
    def make_callback(disp, pts_dict, lbls_dict, win_name):
        def callback(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                print(f"[{win_name}] Obj {obj_id} + point:", x, y)
...
def main():
    ...
    while True:
        ...
        if not if_init:
            ...
            if len(out_obj_ids_top) + len(out_obj_ids_front)>=2:  # Make sure we have at least two objects in total

                mask_dict = {obj_id:{"top": None, "front": None} for obj_id in ann_obj_id}
                save_mask_dict = {obj_id:{"top": None, "front": None} for obj_id in ann_obj_id}
                ...
                if has_nonempty_masks(save_mask_dict, view='top') and \
                    ...
                    episode_step_data = {'full_top_frame_rgb': frame_rgb_top,
                    ...
                    for i, (obj_id, views) in enumerate(save_mask_dict.items()):
                        if i == 0:
                            key_name = "object"
                        else:
                            key_name = f"object_{i}" if i != len(save_mask_dict)-1 else "gripper"
                            
                        if views["top"] is not None:
                            top_camera_object_current_points = views["top"]
                            top_camera_object_current_contours = top_camera_object_current_points.reshape(-1, 1, 2).astype(np.int32)
                            # print("top_camera_object_current_points.shape", top_camera_object_current_points.shape)
                            episode_step_data[f'{key_name}_top_contour'] = np.asarray(top_camera_object_current_contours, dtype=np.float32)
                        if views["front"] is not None:
                            front_camera_object_current_points = views["front"]
                            front_camera_object_current_contours = front_camera_object_current_points.reshape(-1, 1, 2).astype(np.int32)
                ...
                for i, (obj_id, views) in enumerate(mask_dict.items()):
                    if views["top"] is not None:
                        top_camera_object_out_mask = views["top"]
                        top_camera_object_out_mask = cv2.cvtColor(top_camera_object_out_mask, cv2.COLOR_GRAY2RGB)
                        top_camera_object_out_mask[:, :, i] = np.clip(top_camera_object_out_mask[:, :, i] * 255, 0, 255).astype(np.uint8)
                        frame_rgb_top = cv2.addWeighted(frame_rgb_top, 0.9, top_camera_object_out_mask, 0.7, 0)
                    if views["front"] is not None:
                        front_camera_object_out_mask = views["front"]
                        front_camera_object_out_mask = cv2.cvtColor(front_camera_object_out_mask, cv2.COLOR_GRAY2RGB)
                        front_camera_object_out_mask[:, :, i] = np.clip(front_camera_object_out_mask[:, :, i] * 255, 0, 255).astype(np.uint8)
                        frame_rgb_front = cv2.addWeighted(frame_rgb_front, 0.9, front_camera_object_out_mask, 0.8, 0)
    ...
    # Stop the robot thread if it's running
