...
def create_fake_episode(path):
    ...
    for step in range(EPISODE_LENGTH):
        episode.append({
            ...
            'state': np.asarray(np.random.rand(16), dtype=np.float32),
            'action': np.asarray(np.random.rand(1), dtype=np.float32),
            'language_instruction': 'dummy instruction',
            # 'is_intervene': np.asarray([np.random.choice([True, False])], dtype=np.bool_),
        })
